const validationRules = {
    ...
    'password': { required: true, textOnly: false, minLength: 8, maxLength: 128, specialChar: true },
    'checkbox': { check: true }
}
...
const checkValidity = (value, rules) => {
    ...
    if(rules.minLength) {
        ...
        errorMsg = !errorMsg && !isValid ? `Must be more than ${rules.minLength} characters`: errorMsg;
    }
    if(rules.maxLength) {
        isValid = value.length <= rules.maxLength && isValid;
        errorMsg = !errorMsg && !isValid ? `Must be less than ${rules.maxLength} characters`: errorMsg;
    }
    if(rules.specialChar) {
        isValid = hasSpecialCharacters(value) && isValid;
        errorMsg = !errorMsg && !isValid ? `Must have at least one special character (!@#$%&*^)`: errorMsg;
    }
    if(rules.matchInput) {
