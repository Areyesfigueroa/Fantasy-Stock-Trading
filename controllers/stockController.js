...
exports.searchBySymbol = async (request, response) => {
  try {
    ...
    response
      .status(500)
      .send(new StockErrorHandler(`Search Failed: ${errorMessage}`))
  }
}
...
exports.getShareUnits = async (request, response) => {
  try {
    ...
    response
      .status(500)
      .send(
        new StockErrorHandler(`Could not get share units: ${error.message}`)
      )
  }
}
...
exports.buyShares = async (request, response) => {
  try {
    ...
    const newBalance = await buySharesService.calculateNewBalance(
      user.user_id,
      body.unitPrice,
      body.shareUnits
    )
    await buySharesService.updateHoldings(
      user.user_id,
      body.symbol,
      body.shareUnits,
      newBalance
    ...
    response
      .status(500)
      .send(
        new StockErrorHandler(`Could not update your stocks: ${error.message}`)
      )
  }
}
...
exports.sellShares = async (request, response) => {
  try {
    ...
    await sellSharesService.calculateSavedShares(
      user.user_id,
      body.symbol,
      body.shareUnits
    )
    await sellSharesService.updateShareUnits(
      user.user_id,
      body.symbol,
      body.shareUnits
    )
    await sellSharesService.updateAccountBalance(
      user.user_id,
      body.unitPrice,
      body.shareUnits
    )
    ...
    response
      .status(500)
      .send(
        new StockErrorHandler(`Could not reduce the shares: ${error.message}`)
      )
  }
}
...
exports.getStocks = async (request, response) => {
  try {
    ...
    response
      .status(500)
      .send(
        new StockErrorHandler(
          `Could not retrive saved stocks: ${error.message}`
        )
      )
  }
}
