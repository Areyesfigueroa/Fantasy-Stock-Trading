...
exports.register = async (request, response) => {
  ...
  try {
    ...
    const userSession = await registerUserService.registerUser(
      body.email,
      body.firstName,
      body.lastName,
      body.password,
    ...
    response
      ...
      .send(
        new StockErrorHandler(
          `Server Error, could not register: ${err.message}`
        )
      )
  }
}
...
exports.login = async (request, response) => {
  ...
  try {
    ...
    const userSession = await loginUserService.loginUser(
      body.email,
      body.password
    )
    response.send(userSession)
    ...
    response
      .status(500)
      .send(
        new StockErrorHandler(`Server Error, could not login: ${err.message}`)
      )
  }
}
...
exports.logout = async (request, response) => {
  try {
    ...
    response
      ...
      .send(
        new StockErrorHandler(
          `Server error, could not logout: ${error.message}`
        )
      )
  }
}
